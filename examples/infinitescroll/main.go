...
func init() {
	...
	bgImage, _, err = ebitenutil.NewImageFromFile("../../assets/space.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatal(err)
	}
}
...
func update(screen *ebiten.Image) error {
	...
	ebitenutil.DebugPrint(screen, fmt.Sprintf("WHAT %d/%d=%f, %d/%d=%f", screenWidth, w, float64(screenWidth/w), screenHeight, h, float64(screenHeight/h)))
	// ebitenutil.DebugPrint(screen, fmt.Sprintf("TPS: %0.2f\n ", ebiten.CurrentTPS()))
	// ebitenutil.DebugPrint(screen, fmt.Sprintf("TPS: %0.2f\n bgImage.Size().width: %d\n bdImage.Size().height: %d", ebiten.CurrentTPS(), w, h))
	return nil
}
